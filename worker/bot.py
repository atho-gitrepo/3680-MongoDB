...
db_manager = None # Renamed from firebase_manager to be generic
...
# Use the MongoDB URI instead of Firebase JSON
...
class MongoDBManager:
    ...
    def __init__(self, mongo_uri: str):
        ...
        self.UNRESOLVED_BETS_COLLECTION = None
        self.RESOLVED_BETS_COLLECTION = None
        ...
        try:
            ...
            if not mongo_uri:
                logger.warning("MONGO_URI is empty. Skipping MongoDB initialization.")
                return
            ...
            # Use the database specified in the URI, or fall back to 'betting_data'
            DB_NAME = client.options.get('database', 'betting_data')
            self.db = client[DB_NAME]
            
            self.MATCHES_COLLECTION = self.db["tracked_matches"]
            self.UNRESOLVED_BETS_COLLECTION = self.db["unresolved_bets"]
            self.RESOLVED_BETS_COLLECTION = self.db["resolved_bets"]
    ...
    def is_bet_unresolved(self, match_id: int or str) -> bool:
        ...
        # 2. Perform a direct MongoDB lookup (minimal read operation)
        try:
            # Use the stored match_id as the unique identifier for the document
            doc = self.UNRESOLVED_BETS_COLLECTION.find_one({'_id': match_id_str}, projection={'_id': 1})
            if doc:
                # Update cache if found in DB
                # Note: We don't fetch the full document here, only checking existence
                return True
            return False
        except Exception as e:
            logger.error(f"MongoDB Error during is_bet_unresolved: {e}")
            return False
    ...
    def get_unresolved_bet_data(self, match_id: int or str) -> dict or None:
        ...
        # 2. Perform a direct MongoDB lookup
        try:
            doc = self.UNRESOLVED_BETS_COLLECTION.find_one({'_id': match_id_str})
            if doc:
                # MongoDB uses '_id', which we set as match_id_str. 
                # We can remove it before caching/returning if the rest of the code expects a clean dict.
                # However, for MongoDB, it's best to leave it.
                data = doc
            ...
            logger.error(f"MongoDB Error during get_unresolved_bet_data: {e}")
            return None
    ...
    def get_tracked_match(self, match_id):
        if not self.db: return None
        try:
            # Use match_id as the document's _id
            doc = self.MATCHES_COLLECTION.find_one({'_id': str(match_id)})
            return doc if doc else None
        except Exception as e:
            logger.error(f"MongoDB Error during get_tracked_match: {e}")
            return None
    ...
    def update_tracked_match(self, match_id, data):
        ...
        match_id_str = str(match_id)
        try:
            # Use upsert=True to create if it doesn't exist, and $set to merge fields
            ...
            logger.error(f"MongoDB Error during update_tracked_match: {e}")
            
    def delete_tracked_match(self, match_id):
        if not self.db: return
        try:
            self.MATCHES_COLLECTION.delete_one({'_id': str(match_id)})
        except Exception as e:
            logger.error(f"MongoDB Error during delete_tracked_match: {e}")
    ...
    def get_stale_unresolved_bets(self, minutes_to_wait=BET_RESOLUTION_WAIT_MINUTES):
        ...
        try:
            ...
            # Filter by bet_type (assuming only 'regular' is skipped at this stage)
            # and by placed_at time.
            query = {
                'bet_type': {'$ne': BET_TYPE_REGULAR}, # Exclude regular bets (resolved at HT)
                'placed_at': {'$lt': time_threshold}
            }
            
            stale_bets = {}
            # Fetch all documents matching the query
            for doc in self.UNRESOLVED_BETS_COLLECTION.find(query):
                # The document ID is the match_id_str
                stale_bets[doc['_id']] = doc
                
            return stale_bets
        except Exception as e:
            logger.error(f"MongoDB Error during get_stale_unresolved_bets: {e}")
            return {}
    ...
    def add_unresolved_bet(self, match_id, data):
        ...
        try:
            ...
            # In MongoDB, we use the match_id as the document's unique _id for quick lookups
            data['_id'] = match_id_str 
            self.UNRESOLVED_BETS_COLLECTION.insert_one(data)
            # Update cache when a bet is added
            self._unresolved_bets_cache[match_id_str] = data 
        except pymongo.errors.DuplicateKeyError:
            # This is fine, means the bet was already inserted but perhaps cache wasn't updated
            logger.info(f"Duplicate bet key error for {match_id_str}. Bet already exists.")
        except Exception as e:
            logger.error(f"MongoDB Error during add_unresolved_bet: {e}")
    ...
    def move_to_resolved(self, match_id, bet_info, outcome):
        ...
        try:
            ...
            # 1. Insert into resolved_bets
            self.RESOLVED_BETS_COLLECTION.insert_one(resolved_data)
            
            # 2. Delete from unresolved_bets
            self.UNRESOLVED_BETS_COLLECTION.delete_one({'_id': match_id_str})
            
            # 3. Clear from cache
            self._unresolved_bets_cache.pop(match_id_str, None) 
            return True
        except Exception as e:
            logger.error(f"MongoDB Error during move_to_resolved: {e}")
            return False

    def get_last_api_call(self):
        if not self.db: return None
        try:
            # We use a fixed document ID for config settings
            doc = self.CONFIG_COLLECTION.find_one({'_id': 'api_tracker'})
            if doc and 'last_resolution_api_call' in doc:
                return doc['last_resolution_api_call']
            return None
        except Exception as e:
            logger.error(f"MongoDB Error during get_last_api_call: {e}")
            return None
    ...
    def update_last_api_call(self):
        ...
        try:
            ...
            self.CONFIG_COLLECTION.update_one(
                {'_id': 'api_tracker'},
                {'$set': {'last_resolution_api_call': timestamp}},
                upsert=True # Create the document if it doesn't exist
            )
        except Exception as e:
            logger.error(f"MongoDB Error during update_last_api_call: {e}")
...
def initialize_bot_services():
    """Initializes all external services (MongoDB Manager and Sofascore Client)."""
    global db_manager
    ...
    try:
        ...
        logger.critical("Bot cannot proceed. MongoDB initialization failed.")
        return False
        
    if db_manager is None or not db_manager.db:
         logger.critical("Bot cannot proceed. MongoDB initialization failed.")
         return False

...
class SofascoreService:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.session = None
        self.fotmob: FotMobProvider | None = None
        self._init_session()
    ...
    def get_live_events(self):
        """Fetch live football using FotMob first, then legacy providers.

        Hosted IPs can be blocked by SofaScore/LiveScore. FotMob is therefore
        the primary source; the old providers remain as compatibility fallbacks.
        """
        if self.fotmob:
            try:
                date = get_today()
                raw = self.fotmob.get_live_matches(date)
                if raw:
                    normalized = [self.fotmob.normalize_event(m) for m in raw]
                    self.logger.info("⚽ FotMob live feed: %d matches", len(normalized))
                    return parse_events(normalized)
                self.logger.warning("⚠️ FotMob live feed empty; trying legacy providers.")
            except Exception as exc:
                self.logger.warning("FotMob live fetch failed: %s", exc)
        ...
        try:
            ...
            self.logger.error(f"❌ All live data providers failed: {e}")
            return []

    def get_fotmob_match_details(self, event_id: int | str) -> dict:
        """Return raw FotMob details or an empty dict."""
        if not self.fotmob:
            return {}
        try:
            data = self.fotmob.get_match_details(event_id)
            return data if isinstance(data, dict) else {}
        except Exception as exc:
            self.logger.warning("FotMob match details failed for %s: %s", event_id, exc)
            return {}

    def get_live_features(self, event_id: int | str) -> dict:
        details = self.get_fotmob_match_details(event_id)
        if not details:
            return {}
        try:
            return self.fotmob.extract_features(details)
        except Exception as exc:
            self.logger.warning("FotMob feature extraction failed for %s: %s", event_id, exc)
            return {}

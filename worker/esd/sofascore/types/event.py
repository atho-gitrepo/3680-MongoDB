...
def _parse_fotmob_event(data: dict) -> Event:
    """Translate the normalized FotMob event shape into the shared Event model."""
    tournament = data.get("tournament", {}) or {}
    category = tournament.get("category", {}) or {}
    status = data.get("status", {}) or {}
    return Event(
        id=int(data.get("id", 0) or 0),
        start_timestamp=int(data.get("startTimestamp", 0) or 0),
        slug=data.get("slug", ""),
        tournament=parse_tournament(tournament),
        time=TimeEvent(current_period_start=int(data.get("startTimestamp", 0) or 0)),
        status_time=StatusTime(),
...
def parse_event(data: dict) -> Event:
    ...
    # FotMob normalized event format
    if data.get("_provider") == "fotmob":
        return _parse_fotmob_event(data)

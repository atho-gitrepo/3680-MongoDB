...
class SofascoreClient:
    ...
    def get_live_features(self, event_id: int | str) -> dict:
        """Fetch live xG/shots/corners/chance/card features from FotMob."""
        if not self.service:
            self.logger.error("Service not initialized. Cannot get live features.")
            return {}
        return self.service.get_live_features(event_id)

    def get_fotmob_match_details(self, event_id: int | str) -> dict:
        """Fetch raw FotMob details through the service facade."""
        if not self.service:
            self.logger.error("Service not initialized. Cannot get FotMob details.")
            return {}
        return self.service.get_fotmob_match_details(event_id)

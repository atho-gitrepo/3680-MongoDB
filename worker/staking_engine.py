...
===================
Flat stake: $10.
Pauses after 3 consecutive losses for 1 hour.
...
Key Features:
- Flat $10 stake for every qualifying signal
...
# The latest strategy uses a flat stake. Keep the list interface for callers.
STAKE_SEQUENCE = [10.0]

# Safety thresholds
MAX_CONSECUTIVE_LOSSES = int(os.getenv("MAX_CONSECUTIVE_LOSSES", "3"))
PAUSE_DURATION = int(os.getenv("PAUSE_DURATION", "3600"))

# Odds used for staking statistics. Actual market odds are stored per bet.
ODDS = float(os.getenv("ODDS", "2.0"))
...
class StakingEngine:
    ...
    """
    Manages flat staking for the Under 0.5 HT strategy.
    Tracks consecutive losses, pause state, bankroll, and statistics.
    """
    ...
    def record_result(self, is_win: bool, match_info: Optional[Dict] = None) -> Dict:
        ...
        if is_win:
            ...
            self.current_step = 0
            logger.debug(f"✅ WIN: Stake ${stake:.2f} | Profit +${profit:.2f} | Bankroll: ${self.current_bankroll:.2f}")
            ...
            self.current_step = 0
            logger.debug(f"❌ LOSS: Stake ${stake:.2f} | Loss -${loss:.2f} | Bankroll: ${self.current_bankroll:.2f}")
    ...
    def get_status_message(self) -> str:
        ...
        message += (
            ...
            f"⚠️ Flat stake: ${STAKE_SEQUENCE[0]:.2f} | Pause after {MAX_CONSECUTIVE_LOSSES} consecutive losses\n"
